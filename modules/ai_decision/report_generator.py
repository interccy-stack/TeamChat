...
logger = logging.getLogger("qwenpaw.ai_decision.report")
...
class ReportGenerator:
    ...
    def _generate_summary(self, vote) -> Dict:
        winner = vote.winner
        total_weighted = sum(o.weighted_votes for o in vote.config.options)
        if not winner:
            return {"conclusion": "投票未完成", "winner": None, "confidence": "N/A", "risk_level": "N/A"}
        winner_ratio = winner.weighted_votes / total_weighted if total_weighted > 0 else 0
        confidence = "high" if winner_ratio >= 0.6 and vote.consensus_level >= 0.7 else \
                    "medium" if winner_ratio >= 0.5 and vote.consensus_level >= 0.5 else "low"
        risk = "low" if vote.consensus_level >= 0.8 else "medium" if vote.consensus_level >= 0.5 else "high"
        return {
            "conclusion": f"推荐方案：{winner.text}",
            "winner": {"id": winner.id, "text": winner.text, "votes": winner.votes,
                       "weighted_votes": round(winner.weighted_votes, 2), "ratio": f"{winner_ratio:.1%}"},
    ...
    def _generate_vote_details(self, vote) -> Dict:
        ...
        for i, option in enumerate(vote.config.options):
            ...
            options.append({
                "rank": rank, "medal": medal, "id": option.id, "text": option.text,
                "votes": option.votes, "weighted_votes": round(option.weighted_votes, 2),
                "supporters": supporters, "total_support_weight": round(total_weight, 2),
                "avg_confidence": round(avg_confidence, 2)
            })
        options.sort(key=lambda x: x["weighted_votes"], reverse=True)
        return {
            "total_options": len(vote.config.options), "total_votes": len(vote.votes),
    ...
    def _generate_consensus_analysis(self, vote) -> Dict:
        ...
        if level >= 0.8:
            desc = "高度共识，决策风险低"
            factors = ["多数智能体支持同一方案", "分歧得到有效解决", "各角色专业判断趋于一致"]
        elif level >= 0.6:
            desc = "基本共识，存在轻微分歧"
            factors = ["主要智能体达成一致", "少数智能体持保留意见", "建议关注潜在风险点"]
        elif level >= 0.4:
            desc = "共识度较低，决策存在风险"
            factors = ["智能体间分歧较大", "可能需要进一步讨论", "建议重新评估方案"]
        else:
            desc = "共识度很低，不建议执行"
            factors = ["智能体间存在严重分歧", "方案可能不满足多方需求", "强烈建议重新设计投票"]
        return {"level": round(level, 2), "percentage": f"{level:.1%}", "description": desc, "factors": factors}
    ...
    def _generate_disagreements(self, vote) -> List[Dict]:
        ...
        for d in disagreements:
            ...
            detailed.append({
                ...
                "severity": d["severity"], "support_count": d["support_count"],
                "oppose_count": d["oppose_count"],
                "support_reasons": support_reasons[:3],
                "oppose_reasons": oppose_reasons[:3],
                "recommendation": self._resolve_disagreement(d)
            })
        return detailed
    
    def _resolve_disagreement(self, d: Dict) -> str:
        if d["severity"] == "high":
            return "建议组织专项讨论，深入分析分歧原因"
        elif d["severity"] == "medium":
            return "建议在执行过程中关注相关风险"
    ...
    def _generate_recommendations(self, vote) -> List[Dict]:
        ...
        if winner:
            recs.append({
                "category": "执行建议", "priority": "high",
                "content": f"采用「{winner.text}」作为实施方案",
                "rationale": f"该方案获得{winner.votes}票支持，加权得票{winner.weighted_votes:.2f}"
            })
        if vote.consensus_level < 0.6:
            recs.append({"category": "风险提示", "priority": "high",
                        "content": "共识度较低，建议补充说明或调整方案",
                        "rationale": f"当前共识度仅{vote.consensus_level:.1%}"})
        high_weight_dissenters = [v for v in vote.votes if v.option_id != (winner.id if winner else "") and v.weight > 0.2]
        if high_weight_dissenters:
            names = ", ".join([v.agent_name for v in high_weight_dissenters[:2]])
            recs.append({"category": "协调建议", "priority": "medium",
                        "content": f"关注{names}等权重较高的反对意见",
    ...
    def _generate_risk_assessment(self, vote) -> Dict:
        risks = []
        if vote.consensus_level < 0.6:
            risks.append({"type": "共识风险", "level": "high", "description": "智能体间分歧较大",
                         "mitigation": "组织补充讨论，明确执行细节"})
        elif vote.consensus_level < 0.8:
            risks.append({"type": "共识风险", "level": "medium", "description": "部分智能体持保留意见",
                         "mitigation": "执行中持续沟通"})
        low_conf = [v for v in vote.votes if v.confidence < 0.6]
        if len(low_conf) > len(vote.votes) / 3:
            risks.append({"type": "决策质量风险", "level": "medium",
                         "description": f"{len(low_conf)}个智能体置信度较低",
        ...
        if any(r["level"] == "high" for r in risks):
            ...
            overall = "medium"
        return {"overall_risk": overall, "risk_count": len(risks), "risks": risks}
    
    def _generate_next_steps(self, vote) -> List[Dict]:
        steps = [
            {"phase": "immediate", "timeframe": "24小时内",
             "actions": [f"确认采用「{vote.winner.text if vote.winner else '待定'}」方案",
                        "通知所有智能体投票结果", "准备执行计划"]},
            {"phase": "short_term", "timeframe": "1周内",
             "actions": ["制定详细实施方案", "分配任务和责任", "建立进度跟踪机制"]},
            {"phase": "medium_term", "timeframe": "1个月内",
             "actions": ["定期评估执行效果", "收集反馈意见", "必要时调整方案"]}
        ]
        if vote.consensus_level < 0.6:
            steps.insert(1, {"phase": "immediate", "timeframe": "48小时内",
                            "actions": ["组织补充讨论会", "听取反对意见", "寻求妥协方案"]})
        return steps
...
"""
        for opt in report['vote_details']['options']:
            md += f"| {opt['rank']} | {opt['text']} | {opt['votes']} | {opt['weighted_votes']} |\n"
        md += f"\n## 共识度分析\n**共识度: {report['consensus_analysis']['percentage']}**\n{report['consensus_analysis']['description']}\n\n影响因素:\n"
        for f in report['consensus_analysis']['factors']:
            md += f"- {f}\n"
        md += "\n## AI建议\n\n"
